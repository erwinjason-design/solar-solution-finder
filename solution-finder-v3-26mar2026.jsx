...
function calculateResults(inputs) {
  ...
  const spaceNum = Number(inputs.availableSpace) || 0;
  const schefflerKnockout = spaceNum < A.SCHEFFLER_MIN_SPACE_M2 || inputs.shading === "no";
  const schefflerConditional = inputs.shading === "partial";

  // Step 6: Current baseline costs
  const monthlyFuelCost = Number(inputs.monthlyFuelCost) || 0;
  // School-year annual cost: monthly × number of school months
  const schoolMonths = schoolWeeks * 7 / 30; // ~8.87 for 38 weeks
  const annualFuelCost = monthlyFuelCost * schoolMonths;
  const currentCostPerMeal = annualMeals > 0 ? annualFuelCost / annualMeals : 0;

  // Step 7: Baseline CO2
  let annualCO2Baseline = 0;
  if (inputs.fuelType === "lpg" || inputs.fuelType === "mixed") {
    // For LPG or mixed: estimate LPG kg from cylinders if provided, otherwise from cost
    const lpgMonthlyKg = inputs.lpgCylinders ? inputs.lpgCylinders * 13
      : (monthlyFuelCost / (inputs.country === "kenya" ? 250 : 2500));
    const lpgFraction = inputs.fuelType === "mixed" ? 0.5 : 1.0; // assume 50/50 for mixed
    annualCO2Baseline += lpgMonthlyKg * lpgFraction * (schoolWeeks * 7 / 30) * A.LPG_CO2_KG_PER_KG;
  }
  if (inputs.fuelType === "firewood" || inputs.fuelType === "charcoal" || inputs.fuelType === "mixed") {
    const biomassMonthlyKg = inputs.biomassKg || 0;
    if (biomassMonthlyKg > 0) {
      annualCO2Baseline += biomassMonthlyKg * (schoolWeeks * 7 / 30) * A.BIOMASS_CO2_KG_PER_KG;
    } else if (inputs.fuelType === "mixed") {
      // If mixed but no biomass kg entered, estimate from cost (other half of fuel spend)
      const biomassEstKg = (monthlyFuelCost * 0.5) / (inputs.country === "kenya" ? 15 : 250); // rough KES or TZS per kg firewood
      annualCO2Baseline += biomassEstKg * (schoolWeeks * 7 / 30) * A.BIOMASS_CO2_KG_PER_KG;
    }
  }
  if (inputs.fuelType === "electric") {
    // Grid electricity CO2: ~0.4 kg CO2/kWh for East Africa grid mix (estimate)
    const monthlyKWh = monthlyFuelCost / (inputs.country === "kenya" ? 25 : 400); // rough cost per kWh
    annualCO2Baseline = monthlyKWh * (schoolWeeks * 7 / 30) * 0.4;
  }
  ...
  if (!schefflerKnockout && !ghiKnockout) {
    ...
    const annualBackupFuel = annualFuelCost * (1 - solarFraction);
    ...
    // Savings
    const annualSavings = annualFuelCost * solarFraction - annualOpex;
  ...
  if (!ghiKnockout) {
    ...
    const pvSizeKW = dailyEnergyKWh / (ghi * 0.80); // 0.80 = system derate (temp, wiring, inverter, soiling losses)
  ...
  if (!ghiKnockout) {
    ...
    const pvSizeKW = dailyEnergyKWh * 0.5 / (ghi * 0.80); // 50% of demand, 0.80 system derate
    const pvSizeRounded = Math.ceil(pvSizeKW);
  ...
  // Only meaningful when Scheffler is eligible (provides the alternative cooking option)
  const schefflerSolution = solutions.find(s => s.id === "scheffler_lpg");
  if (schefflerSolution) {
    const schefflerCapex = schefflerSolution.capex;
    const generalPVCostPerKW = kesToLocal(A.PV_COST_PER_KW_KES * 1.25, curr);
  ...
  // Baseline 30-year TCO with fuel inflation (same treatment as solar solutions)
  let baselineTCO30 = 0;
  for (let y = 1; y <= 30; y++) {
    baselineTCO30 += annualFuelCost * Math.pow(1 + A.FUEL_INFLATION, y);
  }
  ...
  return {
    ...
    baselineTCO30: Math.round(baselineTCO30),
...
const T = {
  en: {
    ...
    costPerMealLabel: "Energy cost per meal",
    co2PerYear: "CO₂ per year",
    recommendation: "Recommendation",
    solutionComparison: "Solution Comparison",
    solution: "Solution",
    capex: "Upfront cost",
    costPerMeal: "Fuel cost/meal",
    payback: "Payback",
    tco30yr: "Total cost (30yr)",
    solarPct: "Solar share",
    co2Saved: "CO₂ reduced/yr",
    best: "BEST",
...
function ProgressBar({ step, totalSteps, lang }) {
  ...
  return (
    <div style={{ padding: "16px 0" }}>
      <div style={{
        ...
        <span>{lang === "sw" ? `Hatua ${step} ya ${totalSteps}` : `Step ${step} of ${totalSteps}`}</span>
        <span>{Math.round(pct)}% {lang === "sw" ? "imekamilika" : "complete"}</span>
      </div>
      <div style={{
...
function Card({ children, style }) {
  ...
  return (
    <div style={{
      ...
      padding: mobile ? 16 : 24, marginBottom: 16, ...style,
    }}>
      {children}
    </div>
  );
...
function Label({ children, sub, required, optional, lang }) {
  return (
    <div style={{ marginBottom: sub ? 4 : 12 }}>
      <div style={{ fontWeight: 600, fontSize: 15, color: colors.text, marginBottom: 2 }}>
        {children}
        {required && <span style={{ color: colors.danger, marginLeft: 3 }}>*</span>}
        {optional && <span style={{ fontSize: 12, fontWeight: 400, color: colors.textSecondary, marginLeft: 6 }}>({lang === "sw" ? "si lazima" : "optional"})</span>}
      </div>
...
function InfoTip({ term, lang }) {
  const [open, setOpen] = useState(false);
  const tips = {
    scheffler: {
      en: "A Scheffler reflector is a large parabolic mirror that concentrates sunlight to a fixed focal point, creating intense heat for cooking. It tracks the sun automatically and can reach temperatures above 300°C.",
      sw: "Kioo cha Scheffler ni kioo kikubwa cha paraboliki kinachokusanya mwanga wa jua kwenye sehemu moja, na kutengeneza joto kali kwa kupikia. Kinafuatilia jua kiotomatiki.",
    },
    induction: {
      en: "Induction cooking uses electromagnetic fields to heat pots directly. It's 85-90% efficient (vs 60% for regular electric hotplates) but requires magnetic cookware — stainless steel or cast iron. Aluminium pots don't work.",
      sw: "Kupikia kwa induction kunatumia uwanja wa sumaku kupasha joto sufuria moja kwa moja. Ni bora zaidi (85-90%) lakini inahitaji sufuria za chuma cha pua au cast iron. Sufuria za alumini hazifanyi kazi.",
    },
    solarFraction: {
      en: "The percentage of your total cooking energy that comes from solar instead of fuel. A 50% solar fraction means half your cooking energy is free from the sun.",
      sw: "Asilimia ya nishati yako ya kupikia inayotoka kwa jua badala ya mafuta. Sehemu ya jua ya 50% inamaanisha nusu ya nishati yako ya kupikia ni bure kutoka kwa jua.",
    },
    payback: {
      en: "The number of years until fuel savings pay back the initial investment cost. After payback, the system saves you money every year.",
      sw: "Idadi ya miaka hadi akiba ya mafuta irudishe gharama ya awali ya uwekezaji. Baada ya kurejesha, mfumo unakuokoa pesa kila mwaka.",
    },
    tco: {
      en: "Total Cost of Ownership over 30 years: includes the upfront cost, all maintenance, replacement parts, and remaining fuel costs.",
      sw: "Gharama ya Jumla ya Umiliki kwa miaka 30: inajumuisha gharama ya awali, matengenezo yote, vipuri, na gharama za mafuta zilizobaki.",
    },
    energyCostPerMeal: {
      en: "This is the cost of cooking fuel/energy per meal served. It does NOT include the cost of food ingredients — only the energy to cook them.",
      sw: "Hii ni gharama ya mafuta/nishati ya kupikia kwa kila mlo unaotolewa. HAIJUMUISHI gharama ya viungo vya chakula — nishati tu ya kupikia.",
    },
  };
  const tip = tips[term];
  if (!tip) return null;
  return (
    <span style={{ position: "relative", display: "inline-block", marginLeft: 4 }}>
      <span onClick={() => setOpen(!open)} style={{
        ...
        width: 18, height: 18, borderRadius: 9, background: colors.primaryLight,
        color: colors.primary, fontSize: 11, fontWeight: 700, cursor: "pointer",
        border: `1px solid ${colors.primary}40`, userSelect: "none",
      }}>?</span>
      {open && (
        <div style={{
          position: "fixed", top: "50%", left: "50%", transform: "translate(-50%, -50%)",
          zIndex: 1000, width: "min(300px, 85vw)",
          padding: "14px 18px", borderRadius: 10, background: "#fff",
          border: `1px solid ${colors.border}`, boxShadow: "0 8px 24px rgba(0,0,0,0.18)",
          fontSize: 14, lineHeight: 1.5, color: colors.text,
        }}>
          {tip[lang] || tip.en}
          <div onClick={() => setOpen(false)} style={{
            marginTop: 8, color: colors.primary, cursor: "pointer", fontSize: 13, fontWeight: 600,
          }}>{lang === "sw" ? "Funga" : "Close"}</div>
        </div>
      )}
...
function CurrencyInput({ value, onChange, placeholder, currency }) {
  // Display with commas, store as number
  const displayValue = value === "" || value === undefined ? "" : Number(value).toLocaleString();
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <input
        ...
        style={{
          width: "100%", padding: "10px 12px 10px 48px", borderRadius: 8,
...
function ProgressiveSummary({ data, step, lang }) {
  if (step < 2) return null;
  const items = [];
  if (data.studentCount) {
    const staff = data.staffCount || Math.round(data.studentCount * 0.05);
    const diners = Math.round((Number(data.studentCount) + Number(staff)) * (data.attendanceRate / 100));
    items.push(lang === "sw" ? `${diners} wanakula/siku` : `${diners} diners/day`);
  }
  if (step >= 3 && data.meals?.length > 0) {
    const mealCount = data.meals.length;
    const days = data.daysPerWeek || 7;
    const weeks = data.schoolWeeks || 38;
    const diners = data.studentCount ? Math.round((Number(data.studentCount) + Number(data.staffCount || Math.round(data.studentCount * 0.05))) * (data.attendanceRate / 100)) : 0;
    const annualMeals = diners * mealCount * days * weeks;
    if (annualMeals > 0) items.push(lang === "sw" ? `~${annualMeals.toLocaleString()} milo/mwaka` : `~${annualMeals.toLocaleString()} meals/yr`);
  }
  if (step >= 4 && data.foods?.length > 0) {
    const hasHigh = data.foods.some(f => FOOD_ENERGY[f]?.category === "high");
    items.push(hasHigh
      ? (lang === "sw" ? "Menyu ya nishati kubwa" : "High-energy menu")
      : (lang === "sw" ? "Menyu ya nishati ya kati" : "Medium-energy menu"));
  }
  if (step >= 5 && data.availableSpace) {
    items.push(`${data.availableSpace}m² ${lang === "sw" ? "nafasi" : "space"}`);
  }
  if (step >= 6 && data.monthlyFuelCost) {
    const curr = CURRENCY[data.country] || CURRENCY.kenya;
    items.push(`${curr.symbol} ${Number(data.monthlyFuelCost).toLocaleString()}/${lang === "sw" ? "mwezi" : "mo"}`);
  }
  if (items.length === 0) return null;
  return (
    <div style={{
      padding: "8px 14px", borderRadius: 8, background: colors.primaryLight,
      fontSize: 13, color: colors.primaryDark, marginBottom: 12,
      display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center",
    }}>
      <span style={{ fontWeight: 600 }}>{lang === "sw" ? "Shule yako:" : "Your school:"}</span>
      {items.map((item, i) => (
        <span key={i} style={{
          padding: "2px 8px", borderRadius: 4, background: "#fff",
...
function Button({ onClick, children, variant = "primary", disabled, style }) {
  const mobile = useIsMobile();
  const base = {
    padding: mobile ? "14px 20px" : "12px 24px", borderRadius: 8, fontSize: 15, fontWeight: 600,
...
function CollapsibleSection({ title, children, defaultOpen = false, lang }) {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div style={{ marginTop: 10 }}>
      <div onClick={() => setOpen(!open)} style={{
        display: "flex", alignItems: "center", gap: 6, cursor: "pointer",
        fontSize: 14, fontWeight: 600, color: colors.primary, padding: "6px 0",
      }}>
        <span style={{ fontSize: 12, transition: "transform 0.2s", transform: open ? "rotate(90deg)" : "rotate(0deg)" }}>▶</span>
        {title}
      </div>
      {open && <div style={{ paddingLeft: 4, paddingTop: 6 }}>{children}</div>}
    </div>
  );
}
...
function NavButtons({ onBack, onNext, nextLabel, nextDisabled = false, lang = "en" }) {
  ...
  return (
    <div style={{
      display: "flex", flexDirection: mobile ? "column-reverse" : "row",
      justifyContent: "space-between", marginTop: 24, gap: mobile ? 10 : 0,
    }}>
      {onBack ? <Button variant="secondary" onClick={onBack} style={mobile ? { width: "100%" } : {}}>{t("back", lang)}</Button> : !mobile && <div />}
      <Button onClick={onNext} disabled={nextDisabled} style={mobile ? { width: "100%" } : {}}>{nextLabel || t("continueBtn", lang)}</Button>
    </div>
  );
}
...
function StepLocation({ data, onChange, onNext, lang }) {
  ...
  return (
    <div>
      ...
      {data.country && (
        <Card>
          <Label sub={t("regionSub", lang)} required>{t("region", lang)}</Label>
...
function StepHeadcount({ data, onChange, onNext, onBack, lang }) {
  return (
    <div>
      ...
      <Card>
        <Label sub={t("numStudentsSub", lang)} required>{t("numStudents", lang)}</Label>
        <Input value={data.studentCount || ""} onChange={v => onChange({ studentCount: v })} placeholder="e.g. 400" />
      </Card>
      <Card>
        <Label sub={t("staffCountSub", lang)} optional lang={lang}>{t("staffCount", lang)}</Label>
        <Input value={data.staffCount || ""} onChange={v => onChange({ staffCount: v })} placeholder={lang === "sw" ? "mf. 20" : "e.g. 20"} />
      </Card>
      <Card>
        <Label required>{t("schoolType", lang)}</Label>
        <Select value={data.schoolType || "boarding"} onChange={v => {
          const att = v === "boarding" ? 100 : v === "day" ? 85 : 90;
          const days = v === "boarding" ? 7 : v === "day" ? 5 : 6;
          onChange({ schoolType: v, attendanceRate: att, daysPerWeek: days });
        }}
          options={[
...
function StepMeals({ data, onChange, onNext, onBack, lang }) {
  ...
  return (
    <div>
      ...
      <Card>
        <Label required>{t("whichMeals", lang)}</Label>
        {Object.entries(MEAL_TIMES).map(([key, m]) => (
          <Checkbox key={key} checked={meals.includes(key)} onChange={() => toggleMeal(key)}
            label={(() => {
              const h = m.hour >= 13 ? m.hour - 12 : m.hour;
              const hInt = Math.floor(h);
              const mins = Math.round((h - hInt) * 60);
              const timeStr = mins > 0 ? `${hInt}:${String(mins).padStart(2, "0")}` : `${hInt}`;
              return `${t(MEAL_KEYS[key], lang)} (~${timeStr}${m.hour >= 12 ? "pm" : "am"})`;
            })()} />
        ))}
      </Card>
      <Card>
...
function StepMenu({ data, onChange, onNext, onBack, lang }) {
  ...
  return (
    <div>
      ...
      <Card>
        <Label required>{t("selectFoods", lang)}</Label>
...
function StepSite({ data, onChange, onNext, onBack, lang }) {
  return (
    <div>
      ...
      <Card>
        <Label required>{lang === "sw" ? "Nafasi ya nje inayopatikana karibu na jikoni" : "Outdoor space available near the kitchen"}</Label>
        <div style={{ fontSize: 13, color: colors.textSecondary, marginBottom: 10 }}>
          {lang === "sw" ? "Fikiria eneo la wazi karibu na jikoni yako:" : "Think of the open area near your kitchen:"}
        </div>
        {[
          { value: "30", label: lang === "sw" ? "Ndogo kuliko darasa (~30m²)" : "Smaller than a classroom (~30m²)" },
          { value: "75", label: lang === "sw" ? "Ukubwa wa darasa (50-100m²)" : "About the size of a classroom (50-100m²)" },
          { value: "200", label: lang === "sw" ? "Nusu ya uwanja wa mpira (100-300m²)" : "About half a football pitch (100-300m²)" },
          { value: "500", label: lang === "sw" ? "Kubwa kuliko uwanja wa mpira (300m²+)" : "Larger than a football pitch (300m²+)" },
        ].map(opt => (
          <label key={opt.value} style={{
            display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", marginBottom: 6,
            borderRadius: 8, cursor: "pointer", fontSize: 14,
            background: String(data.availableSpace) === opt.value ? colors.primaryLight : colors.light,
            border: `1px solid ${String(data.availableSpace) === opt.value ? colors.primary : colors.border}`,
          }}>
            <input type="radio" name="space" checked={String(data.availableSpace) === opt.value}
              onChange={() => onChange({ availableSpace: Number(opt.value) })}
      ...
      <Card>
        <Label required>{t("shading", lang)}</Label>
        <Select value={data.shading || "partial"} onChange={v => onChange({ shading: v })}
          options={[
      ...
      <Card>
        <Label sub={t("electricalNeedsSub", lang)} required>{t("electricalNeeds", lang)}</Label>
...
function StepFuel({ data, onChange, onNext, onBack, lang }) {
  const curr = CURRENCY[data.country] || CURRENCY.kenya;
  const fuelTypes = data.fuelTypes || (data.fuelType ? [data.fuelType] : ["lpg"]);
  const toggleFuel = (f) => {
    const next = fuelTypes.includes(f) ? fuelTypes.filter(x => x !== f) : [...fuelTypes, f];
    if (next.length === 0) return; // must have at least one
    // Also set the legacy fuelType for calculation compatibility
    const primary = next.includes("lpg") ? "lpg" : next.includes("firewood") ? "firewood" : next.includes("charcoal") ? "charcoal" : next[0];
    onChange({ fuelTypes: next, fuelType: next.length > 1 ? "mixed" : primary });
  };
  const hasLPG = fuelTypes.includes("lpg");
  const hasBiomass = fuelTypes.includes("firewood") || fuelTypes.includes("charcoal");
  const cookingOnlyNote = lang === "sw"
    ? "Ingiza mafuta yanayotumika kwa KUPIKIA tu, si kwa kuoga, jenereta, au matumizi mengine."
    : "Enter fuel used for COOKING only, not for showers, generators, or other uses.";
  ...
  return (
    <div>
      ...
      <Card>
        <Label required>{lang === "sw" ? "Aina za mafuta ya kupikia" : "Cooking fuel types"} <span style={{ fontSize: 13, fontWeight: 400, color: colors.textSecondary }}>({lang === "sw" ? "chagua zote zinazotumika" : "select all that apply"})</span></Label>
        <Checkbox checked={fuelTypes.includes("lpg")} onChange={() => toggleFuel("lpg")} label={t("fuelLPG", lang)} />
        <Checkbox checked={fuelTypes.includes("firewood")} onChange={() => toggleFuel("firewood")} label={t("fuelFirewood", lang)} />
        <Checkbox checked={fuelTypes.includes("charcoal")} onChange={() => toggleFuel("charcoal")} label={t("fuelCharcoal", lang)} />
        <Checkbox checked={fuelTypes.includes("electric")} onChange={() => toggleFuel("electric")} label={t("fuelElectric", lang)} />
      </Card>
      {hasLPG && (
        <Card>
          <Label sub={t("lpgCylindersSub", lang)} optional lang={lang}>{t("lpgCylinders", lang)}</Label>
          <Input value={data.lpgCylinders || ""} onChange={v => {
            onChange({ lpgCylinders: v });
            // Auto-estimate monthly cost if user hasn't entered one yet
            if (v && !data.monthlyFuelCost) {
              const pricePerCylinder = data.country === "kenya" ? 3000 : 50000;
              onChange({ lpgCylinders: v, monthlyFuelCost: Number(v) * pricePerCylinder });
            }
          }} placeholder="e.g. 8" />
          {data.lpgCylinders && (
            <div style={{ fontSize: 12, color: colors.textSecondary, marginTop: 6 }}>
              {lang === "sw"
                ? `≈ ${curr.symbol} ${(Number(data.lpgCylinders) * (data.country === "kenya" ? 3000 : 50000)).toLocaleString()}/mwezi (makadirio)`
                : `≈ ${curr.symbol} ${(Number(data.lpgCylinders) * (data.country === "kenya" ? 3000 : 50000)).toLocaleString()}/month (estimate at ${curr.symbol} ${(data.country === "kenya" ? 3000 : 50000).toLocaleString()}/cylinder)`}
            </div>
          )}
        </Card>
      )}
      {hasBiomass && (
        <Card>
          <Label optional lang={lang}>{lang === "sw" ? "Makadirio ya kg za kuni/mkaa kwa mwezi" : "Estimated kg of firewood/charcoal per month"}</Label>
          <Input value={data.biomassKg || ""} onChange={v => onChange({ biomassKg: v })} placeholder="e.g. 2000" />
        </Card>
      )}
      <Card>
        <Label sub={cookingOnlyNote} required>
          {t("monthlyFuelCost", lang, curr.symbol)}
        </Label>
        <CurrencyInput value={data.monthlyFuelCost} onChange={v => onChange({ monthlyFuelCost: v })}
          placeholder={data.country === "kenya" ? "48,000" : "800,000"} currency={curr.symbol} />
      </Card>
      <Card>
        <Label required>{t("potMaterial", lang)} <InfoTip term="induction" lang={lang} /></Label>
        <div style={{ fontSize: 13, color: colors.textSecondary, marginBottom: 8 }}>{t("potMaterialSub", lang)}</div>
  ...
  if (results.ghiKnockout) {
    return (
      <div>
        ...
        <Alert type="danger">
          {lang === "sw" ? (
            <><strong>Rasilimali ya jua haitoshi.</strong> Kulingana na eneo lako ({inputs.region}), viwango vya nishati ya jua ({results.ghi} kWh/m²/siku) viko chini ya kiwango cha chini cha {ASSUMPTIONS.GHI_KNOCKOUT} kWh/m²/siku. Suluhisho za kupikia kwa jua hazitakuwa na thamani katika eneo hili.</>
          ) : (
            <><strong>Solar resource insufficient.</strong> Based on your location ({inputs.region}), solar energy levels ({results.ghi} kWh/m²/day) are below the minimum threshold of {ASSUMPTIONS.GHI_KNOCKOUT} kWh/m²/day. Solar cooking solutions are unlikely to be cost-effective at this site.</>
          )}
        </Alert>
        <Card>
          <p style={{ fontSize: 15, lineHeight: 1.6, color: colors.textSecondary }}>
            {lang === "sw"
              ? "Tunapendekeza kuongea na mshauri wa nishati wa eneo kuhusu kuboresha ufanisi wa LPG au chaguzi za biogas. Bado unaweza kuwasiliana na wauzaji wa ndani hapa chini kwa tathmini ya kitaalamu."
              : "We recommend speaking with a local energy advisor about LPG efficiency improvements or biogas options. You can still contact local vendors below for a professional assessment — they may identify site-specific factors that our screening tool cannot capture."}
          </p>
        </Card>
        <Button onClick={onRestart} variant="secondary">{lang === "sw" ? "Anza Tathmini Mpya" : "Start New Assessment"}</Button>
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{
        ...
        borderRadius: 12, padding: mobile ? 18 : 28, marginBottom: 20, color: "#fff",
      }}>
        ...
        <h2 style={{ fontSize: 24, fontWeight: 700, margin: 0, marginBottom: 4, color: "#fff" }}>
          {inputs.schoolName || (lang === "sw" ? "Shule Yako" : "Your School")}
        </h2>
        <p style={{ fontSize: 14, opacity: 0.85, margin: 0 }}>
          {results.totalDiners} {lang === "sw" ? "wanakula" : "diners"} · {results.annualMeals.toLocaleString()} {lang === "sw" ? "milo/mwaka" : "meals/year"} · {results.ghi} kWh/m²/day
        </p>
      </div>
      ...
      <Card>
        ...
        <div style={{ display: "grid", gridTemplateColumns: mobile ? "1fr" : "1fr 1fr 1fr", gap: mobile ? 12 : 16 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: colors.textSecondary }}>{t("monthlyFuelCostLabel", lang)}</div>
          </div>
          <div>
            <div style={{ fontSize: 22, fontWeight: 700, color: colors.text }}>
              {formatMoney(results.currentCostPerMeal, curr)}
            </div>
            <div style={{ fontSize: 12, color: colors.textSecondary }}>{lang === "sw" ? "Gharama ya nishati kwa mlo" : "Energy cost per meal"} <InfoTip term="energyCostPerMeal" lang={lang} /></div>
          </div>
          <div>
            <div style={{ fontSize: 22, fontWeight: 700, color: colors.text }}>
              {results.annualCO2Baseline.toLocaleString()} kg
            </div>
            <div style={{ fontSize: 12, color: colors.textSecondary }}>{t("co2PerYear", lang)}</div>
          </div>
        </div>
      </Card>
      ...
      {/* What to do next — CTA */}
      <Card style={{ background: colors.primaryLight, border: `1px solid ${colors.primary}40` }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: colors.primaryDark, marginBottom: 8 }}>
          {lang === "sw" ? "Hatua inayofuata" : "What to do next"}
        </div>
        <div style={{ fontSize: 14, lineHeight: 1.6, color: colors.text, marginBottom: 12 }}>
          {lang === "sw"
            ? "Wasiliana na muuzaji kwa tathmini ya eneo na bei kamili. Unaweza pia kushiriki matokeo haya na msimamizi wako wa fedha au bodi ya shule."
            : "Contact a vendor for a site assessment and firm quote. You can also share these results with your bursar or school board to start the conversation."}
        </div>
        <div style={{ padding: 12, background: "#fff", borderRadius: 8, border: `1px solid ${colors.border}` }}>
          <div style={{ fontWeight: 600, marginBottom: 2 }}>Thermofield Industrial</div>
          <div style={{ fontSize: 13, color: colors.textSecondary }}>Scheffler parabolic dishes · Nairobi, Kenya</div>
          <a href="tel:+254721727830" style={{ fontSize: 14, color: colors.primary, fontWeight: 600, textDecoration: "none", display: "inline-block", marginTop: 6 }}>
            📞 +254 (0)721 727 830
          </a>
        </div>
      </Card>
      ...
      <Card>
        <div style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: 1, color: colors.textSecondary, marginBottom: 16 }}>
          {t("solutionComparison", lang)}
        </div>
        {mobile ? (
          /* Mobile: stacked solution cards */
          <div>
            {results.solutions.map((s, i) => (
              <div key={s.id} style={{
                padding: 14, borderRadius: 8, marginBottom: 10,
                background: i === 0 ? colors.primaryLight : colors.light,
                border: `1px solid ${i === 0 ? colors.primary + "40" : colors.border}`,
              }}>
                <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8 }}>
                  ...
                  {s.name}
                  {s.conditional && <span style={{ fontSize: 11, color: colors.warning }}> ⚠️ {t("conditional", lang)}</span>}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 13 }}>
                  <div><span style={{ color: colors.textMuted }}>{t("capex", lang)}:</span> <strong>{formatMoney(s.capex, curr)}</strong></div>
                  <div><span style={{ color: colors.textMuted }}>{t("payback", lang)}:</span> <strong>{s.paybackYears} yrs</strong></div>
                  <div><span style={{ color: colors.textMuted }}>{t("costPerMeal", lang)}:</span> <strong>{formatMoney(s.costPerMeal, curr)}</strong></div>
                  <div><span style={{ color: colors.textMuted }}>{t("solarPct", lang)}:</span> <strong>{s.solarFraction}%</strong></div>
                  <div><span style={{ color: colors.textMuted }}>{t("tco30yr", lang)}:</span> <strong>{formatMoney(s.tco30, curr)}</strong></div>
                  <div><span style={{ color: colors.textMuted }}>{t("co2Saved", lang)}:</span> <strong>{s.co2SavedPerYear.toLocaleString()} kg</strong></div>
                </div>
              </div>
            ))}
            <div style={{
              padding: 14, borderRadius: 8, background: colors.dangerLight,
              border: `1px solid ${colors.danger}30`,
            }}>
              <div style={{ fontWeight: 700, fontSize: 15, marginBottom: 8, color: colors.danger }}>
                {t("current", lang)} ({inputs.fuelType})
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 13 }}>
                <div><span style={{ color: colors.textMuted }}>{t("costPerMeal", lang)}:</span> <strong style={{ color: colors.danger }}>{formatMoney(results.currentCostPerMeal, curr)}</strong></div>
                <div><span style={{ color: colors.textMuted }}>{t("tco30yr", lang)}:</span> <strong style={{ color: colors.danger }}>{formatMoney(results.baselineTCO30, curr)}</strong></div>
              </div>
            </div>
          </div>
        ) : (
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14 }}>
            <thead>
              <tr style={{ borderBottom: `2px solid ${colors.border}` }}>
                <th style={{ textAlign: "left", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("solution", lang)}</th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("capex", lang)}</th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("costPerMeal", lang)}</th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("payback", lang)} <InfoTip term="payback" lang={lang} /></th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("tco30yr", lang)} <InfoTip term="tco" lang={lang} /></th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("solarPct", lang)}</th>
                <th style={{ textAlign: "right", padding: "8px 12px", fontWeight: 600, color: colors.textSecondary }}>{t("co2Saved", lang)}</th>
              </tr>
            </thead>
            <tbody>
              {results.solutions.map((s, i) => (
              ...
              <tr style={{ background: colors.dangerLight }}>
                <td style={{ padding: "10px 12px", fontWeight: 600, color: colors.danger }}>{t("current", lang)} ({inputs.fuelType})</td>
                ...
                <td style={{ textAlign: "right", padding: "10px 12px", color: colors.danger }}>
                  {formatMoney(results.baselineTCO30, curr)}
                </td>
      ...
      {/* Energy Priority — condensed for school admins, expandable for planners */}
      {(results.electricalNeeds === "poor" || results.electricalNeeds === "none") && (
        <Card style={{ borderLeft: `4px solid ${colors.accent}`, background: "#FFFBF5" }}>
          <div style={{ fontSize: 14, lineHeight: 1.6, color: colors.text }}>
            <strong style={{ color: colors.accent }}>{lang === "sw" ? "Kumbuka:" : "Note:"}</strong>{" "}
            {lang === "sw"
              ? "Shule yako ina mahitaji ya umeme ambayo hayajatimizwa. Kupikia kwa Scheffler kunatumia jua moja kwa moja kwa joto na kuacha bajeti yako ya umeme huru kwa taa, kompyuta, na maji."
              : "Your school has unmet electrical needs. Scheffler solar thermal cooks with direct sunlight and leaves your electrical budget free for lighting, computers, and water pumps."}
          </div>
          <CollapsibleSection title={lang === "sw" ? "Maelezo kwa wapangaji" : "Details for planners"} lang={lang}>
            <p style={{ fontSize: 13, lineHeight: 1.6, color: colors.textSecondary, margin: "0 0 8px 0" }}>
            ...
            {results.solutions.filter(s => s.opportunityCost && s.opportunityCost.schefflerCapex > 0).map(s => (
              <div key={s.id + "_opp"} style={{
                ...
                border: `1px solid ${colors.border}`, marginBottom: 6, fontSize: 13,
              }}>
                <div style={{ fontWeight: 600, marginBottom: 4 }}>{s.name}:</div>
                <div style={{ color: colors.textSecondary }}>{t("opportunityCostPV", lang, s.details.pvSizeKW, formatMoney(s.capex, curr))}</div>
                <div style={{ marginTop: 4 }}>→ {t("opportunityCostAlt", lang,
      ...
      {results.solutions.map((s) => (
        <Card key={s.id}>
          <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 4, color: colors.text }}>
            {s.name}
            {s.id === "scheffler_lpg" && <InfoTip term="scheffler" lang={lang} />}
            {(s.id === "pv_battery" || s.id === "pv_lpg") && <InfoTip term="induction" lang={lang} />}
          </h3>
          <p style={{ fontSize: 14, color: colors.textSecondary, lineHeight: 1.5, marginBottom: 8 }}>{s.description}</p>
          <CollapsibleSection title={lang === "sw" ? "Maelezo ya kiufundi" : "Technical details"} lang={lang}>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 16 }}>
            {s.id === "scheffler_lpg" && (
              <>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Sahani za Scheffler" : "Scheffler dishes"}: <strong>{s.details.dishes}</strong></div>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Nafasi inayohitajika" : "Space required"}: <strong>{s.details.spaceRequired}m²</strong></div>
              </>
            )}
            {(s.id === "pv_battery" || s.id === "pv_lpg") && (
              <>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Safu ya PV" : "PV array"}: <strong>{s.details.pvSizeKW} kW</strong></div>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Betri" : "Battery"}: <strong>{s.details.batteryKWh} kWh</strong> (LFP)</div>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Inverter" : "Inverter"}: <strong>{s.details.inverterKW} kW</strong></div>
                <div style={{ fontSize: 13 }}>{lang === "sw" ? "Majiko ya induction" : "Induction burners"}: <strong>{s.details.inductionBurners}</strong></div>
              </>
            )}
            <div style={{ fontSize: 13 }}>{lang === "sw" ? "Mafuta ya akiba/mwaka" : "Annual backup fuel"}: <strong>{formatMoney(s.details.annualBackupFuel, curr)}</strong></div>
            <div style={{ fontSize: 13 }}>{lang === "sw" ? "Matengenezo/mwaka" : "Annual maintenance"}: <strong>{formatMoney(s.details.annualOpex, curr)}</strong></div>
            <div style={{ fontSize: 13 }}>{lang === "sw" ? "Akiba/mwaka" : "Annual savings"}: <strong style={{ color: colors.success }}>{formatMoney(s.annualSavings, curr)}</strong></div>
          </div>
          ...
          {s.requiresInduction && (
            <div style={{
              ...
              <div style={{ fontSize: 13, fontWeight: 700, color: colors.accent, marginBottom: 4 }}>
                {t("inductionRequired", lang)}
              </div>
              <div style={{ fontSize: 13, lineHeight: 1.5, color: colors.text }}>
                {t("inductionDesc", lang)}{" "}
                {lang === "sw" ? "Gharama ya vifaa vya induction ya" : "Induction equipment cost of"}{" "}
                <strong>{formatMoney(s.details.inductionCapex, curr)}</strong>{" "}
                {lang === "sw" ? "imejumuishwa katika gharama ya awali hapo juu." : "is included in the upfront cost above."}
                {s.needNewCookware && (
                  <span> {lang === "sw" ? "Sufuria zako za alumini hazifanyi kazi — gharama ya sufuria mpya imejumuishwa." : "Your current aluminum pots are not compatible — new cookware cost is included."}</span>
                )}
                {!s.needNewCookware && (
                  <span> {lang === "sw" ? "Sufuria zako zinapaswa kufanya kazi — thibitisha kwa jaribio la sumaku." : "Your existing cookware should be compatible — verify with a magnet test (magnet sticks = works)."}</span>
                )}
              </div>
              <div style={{ fontSize: 12, color: colors.textSecondary, marginTop: 6 }}>
                {lang === "sw"
                  ? "Majiko ya induction hudumu ~miaka 6 katika matumizi makubwa ya taasisi. Gharama za kubadilisha zimejumuishwa katika TCO ya miaka 30. Scheffler HAIHITAJI sufuria mpya."
                  : "Induction burners last ~6 years in heavy institutional use. Replacement costs are included in the 30-year TCO. Scheffler solar thermal does NOT require new cookware."}
              </div>
            </div>
          )}
          ...
          <CollapsibleSection title={lang === "sw" ? "Vidokezo vya kupikia" : "Cooking tips"} lang={lang}>
      ...
      {/* Schedule compatibility — collapsible */}
      {inputs.meals && (
        <Card>
          <CollapsibleSection title={t("scheduleNotes", lang)} lang={lang}>
          {inputs.meals.map(m => {
            ...
            return (
              <div key={m} style={{
                ...
                <span><strong>{mt.label}</strong> (~{(() => {
                  const h = mt.hour >= 13 ? mt.hour - 12 : mt.hour;
                  const hInt = Math.floor(h);
                  const mins = Math.round((h - hInt) * 60);
                  return mins > 0 ? `${hInt}:${String(mins).padStart(2, "0")}` : `${hInt}`;
                })()}{mt.hour >= 12 ? "pm" : "am"})</span>
                <span style={{
                  ...
                  {compatible ? t("solarCompatible", lang) : t("backupNeeded", lang)}
                </span>
              </div>
            );
          })}
      ...
      <Card style={{ background: "#F5F5F0" }}>
        ...
        <div style={{ fontSize: 13, lineHeight: 1.7, color: colors.textSecondary }}>
          {lang === "sw" ? (<>
            <p>Takwimu hizi ni makadirio kulingana na taarifa ulizotoa. Ziara ya mtaalamu inahitajika kwa bei kamili.</p>
            <p>Gharama za nishati hubadilika. Takwimu za akiba zinakadiriwa ongezeko la bei ya mafuta la 5% kwa mwaka.</p>
            <p>Mifumo ya Scheffler inahitaji opereta aliyefunzwa. Bajeti ya mafunzo ya opereta katika gharama yako ya jumla.</p>
            <p>Muda wa vipuri vya PV: paneli miaka 30, betri za LFP miaka 10, inverter miaka 10, majiko ya induction miaka 6. Gharama zote za kubadilisha zimejumuishwa katika TCO ya miaka 30.</p>
            <p>Suluhisho za PV zinahitaji vifaa vya kupikia kwa induction. Shule zinazotumia sufuria za alumini zitahitaji sufuria mpya — gharama hii imejumuishwa.</p>
            <p style={{ marginTop: 8, fontStyle: "italic" }}>Makadirio kadhaa muhimu (MJ/mlo kwa aina ya chakula, bei za CAPEX) ni za muda yanahitaji uthibitisho na wataalamu wa kiufundi.</p>
          </>) : (<>
            ...
            <p style={{ marginTop: 8, fontStyle: "italic" }}>Several key assumptions (MJ/meal by food type, CAPEX benchmarks, system efficiencies) are placeholders awaiting validation with ISC/EWB technical partners. Treat all numbers as directional, not definitive.</p>
          </>)}
        </div>
      </Card>
      ...
      {/* Vendor placeholder */}
      <Card>
        <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 8 }}>{t("vendorTitle", lang)}</h3>
        ...
        <p style={{ fontSize: 12, color: colors.textSecondary, marginTop: 8, fontStyle: "italic" }}>
          {t("vendorDisclaimer", lang)}
        </p>
      </Card>

      <div style={{ display: "flex", flexDirection: mobile ? "column" : "row", gap: 10, marginTop: 20 }}>
        <Button onClick={() => {
        ...
        }} variant="accent" style={{ fontSize: 13, padding: "10px 16px" }}>
          {copied ? (lang === "sw" ? "Imenakiliwa! ✓" : "Copied! ✓") : (lang === "sw" ? "Nakili Muhtasari" : "Copy Summary")}
        </Button>
        <Button onClick={() => {
          const text = generateWhatsAppText(results, inputs, lang);
          const encoded = encodeURIComponent(text);
          window.open(`https://wa.me/?text=${encoded}`, "_blank");
        }} variant="primary" style={{ fontSize: 13, padding: "10px 16px", background: "#25D366" }}>
          {lang === "sw" ? "Tuma WhatsApp" : "Share via WhatsApp"}
        </Button>
        <Button onClick={onRestart} variant="secondary" style={{ fontSize: 13, padding: "10px 16px" }}>
          {lang === "sw" ? "Anza Tathmini Mpya" : "New Assessment"}
        </Button>
      </div>
    </div>
  );
...
export default function SolutionFinder() {
  ...
  return (
    <div style={{
      ...
      {/* Header */}
      <div style={{
        background: colors.primaryDark, color: "#fff", padding: mobile ? "10px 14px" : "12px 24px",
        display: "flex", alignItems: "center", justifyContent: "space-between",
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: mobile ? 8 : 12 }}>
          <span style={{ fontSize: mobile ? 20 : 24 }}>☀️</span>
          <div>
            <div style={{ fontSize: mobile ? 14 : 17, fontWeight: 700, letterSpacing: 0.3 }}>{t("appTitle", lang)}</div>
            {!mobile && <div style={{ fontSize: 12, opacity: 0.7 }}>{t("appVersion", lang)} · {t("appSubtitle", lang)}</div>}
          </div>
        </div>
      ...
      {/* Content */}
      <div style={{ maxWidth: 700, margin: "0 auto", padding: mobile ? "12px 12px 60px" : "16px 20px 60px" }}>
        ...
        {step >= 1 && step <= 6 && <ProgressBar step={step} totalSteps={TOTAL_STEPS} lang={lang} />}
        {step >= 2 && step <= 6 && <ProgressiveSummary data={data} step={step} lang={lang} />}
