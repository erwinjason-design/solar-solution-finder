...
function InfoTip({ term, lang }) {
  ...
  return (
    <span style={{ position: "relative", display: "inline-block", marginLeft: 4 }}>
      ...
      {open && (
        <div style={{
          position: "absolute", top: 24, left: -100, zIndex: 100, width: 280,
          padding: "10px 14px", borderRadius: 8, background: "#fff",
          border: `1px solid ${colors.border}`, boxShadow: "0 4px 12px rgba(0,0,0,0.12)",
          fontSize: 13, lineHeight: 1.5, color: colors.text,
        }}>
          {tip[lang] || tip.en}
          <div onClick={() => setOpen(false)} style={{
            marginTop: 6, color: colors.primary, cursor: "pointer", fontSize: 12, fontWeight: 600,
...
function StepSite({ data, onChange, onNext, onBack, lang }) {
  return (
    <div>
      ...
      <Card>
        <Label sub={t("outdoorSpaceSub", lang)} required>{t("outdoorSpace", lang)}</Label>
  ...
  return (
    <div>
      ...
      {results.solutions.map((s) => (
        <Card key={s.id}>
          <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 12, color: colors.text }}>
            {s.name}
      ...
      {inputs.meals && (
        <Card>
          <h3 style={{ fontSize: 17, fontWeight: 700, marginBottom: 12 }}>{t("scheduleNotes", lang)}</h3>
          {inputs.meals.map(m => {
